...
class DrawInAABBCallback : public b2QueryCallback
{
public:
    ...
    virtual bool ReportFixture(b2Fixture* f) {
        ...
        if (b->IsActive() == false)
        {
            world->DrawShape(f, xf, b2Color(0.5f, 0.5f, 0.3f));
        }
        else if (b->GetType() == b2_staticBody)
        {
            world->DrawShape(f, xf, b2Color(0.5f, 0.9f, 0.5f));
        }
        else if (b->GetType() == b2_kinematicBody)
        {
            world->DrawShape(f, xf, b2Color(0.5f, 0.5f, 0.9f));
        }
        else if (b->IsAwake() == false)
        {
            world->DrawShape(f, xf, b2Color(0.6f, 0.6f, 0.6f));
        }
        else
        {
            world->DrawShape(f, xf, b2Color(0.9f, 0.7f, 0.7f));
        }
...
void b2World::DrawDebugDataInAABB(b2AABB &screenAABB) {
    if (g_debugDraw == NULL)
    {
        return;
    }
    ...
    {
        DrawInAABBCallback callback;
        callback.world = this;
        QueryAABB(&callback, screenAABB);
    }
